...
export default {
  ...
  methods: {
    getNormal () {
      this.$http.get(`/accounts/cycle/note_comment/${this.$route.params.id}/`).then(res => {
        this.normalComment = res.data
      }).catch(err => console.log(err.response.data))
    },
    postComment () {
      this.$http.post(`/accounts/cycle/note_comment/${this.$route.params.id}/`, {content: this.commentString}).then(res => {
        this.normalComment = res.data
        this.getNormal()
      }).catch(err => console.log(err.response.data))
    }
