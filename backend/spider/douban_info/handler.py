...
def parser(html,book_url):
    ...
    try:
        ...
        print e
    # try:
    #     cursor.execute(sql,(isbn13, numRaters, average,
    #                          subtitle, author,pubdate,
    #                         origin_title,(img_id), (bingding),
    #                          (tag), (catalog), (pages),
    #                          (d_id), (publisher), (title),
    ...
    conn.close()
    with open('log.txt', 'a') as l:
        l.write(str(isbn13) +"插入数据库===================================================================================成功"+"\n")
        l.close()
    print


def get_proxies():
    f = open('proxies/ippool.txt')
    line = f.readline()
